-- ============================================================
-- MedNet · Migration v5 — is_admin, índices, definir admin
-- Execute no SQL Editor após migration_v4.sql
-- ============================================================

-- ── 1. Função is_admin() — evita recursão em policies ──────
-- SECURITY DEFINER executa como postgres (bypassa RLS),
-- impedindo loop: policy de profiles → query em profiles.
...
-- ── 2. Substituir profiles_admin_update ────────────────────
...
-- ── 4. Definir único usuário atual como admin ───────────────
-- Seguro sem WHERE: único usuário existente no banco.
-- Novos usuários receberão 'operador' por padrão (schema padrão).
